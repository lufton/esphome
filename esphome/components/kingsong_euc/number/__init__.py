...
from esphome.const import CONF_ID, CONF_MIN_VALUE, CONF_MAX_VALUE, CONF_MODE, CONF_STEP, ENTITY_CATEGORY_CONFIG, UNIT_SECOND, UNIT_KILOMETER_PER_HOUR
...
CONFIG_SCHEMA = KINGSONG_EUC_COMPONENT_CONFIG_SCHEMA.extend(
    {
        cv.Optional(number_type): schema.extend({cv.Optional(CONF_MODE, default="BOX"): cv.enum(number.NUMBER_MODES, upper=True)}).extend(cv.polling_component_schema("10s"))
...
async def to_code(config):
    ...
    for number_type, _ in NUMBER_TYPES.items():
        if (conf := config.get(number_type)) and (params := NUMBER_OPTIONS.get(number_type)):
