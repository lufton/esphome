...
CONFIG_SCHEMA = KINGSONG_EUC_COMPONENT_CONFIG_SCHEMA.extend({
    cv.Optional(lock_type): schema.extend(report_interval_schema()) for lock_type, schema in LOCK_TYPES.items()
})
...
async def to_code(config):
    ...
    for lock_type, _ in LOCK_TYPES.items():
        if conf := config.get(lock_type):
            loc = cg.new_Pvariable(conf[CONF_ID], getattr(KingSongEUCLockTypeEnum, lock_type.upper()), conf.get(CONF_REPORT_INTERVAL))
